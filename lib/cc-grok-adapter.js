...
function mergePlainJsonObjects(values) {
...
function unwrapPropertyValue(value, property) {
  const sequence = parseJsonSequence(value);
  for (const item of sequence) {
    if (!isPlainObject(item)) continue;
    if (Object.prototype.hasOwnProperty.call(item, property)) {
      return unwrapPropertyValue(item[property], property);
    }

    const keys = Object.keys(item);
    if (keys.length === 1 && ["arguments", "args", "input", "parameters"].includes(keys[0])) {
      const unwrapped = unwrapPropertyValue(item[keys[0]], property);
      if (unwrapped !== item[keys[0]]) return unwrapped;
    }
  }
  ...
  if (Object.prototype.hasOwnProperty.call(parsed, property)) {
    return unwrapPropertyValue(parsed[property], property);
  }

  const keys = Object.keys(parsed);
  if (keys.length === 1 && ["arguments", "args", "input", "parameters"].includes(keys[0])) {
    return unwrapPropertyValue(parsed[keys[0]], property);
  }

  return value;
}

function coerceValueForSchema(value, schema, property) {
  const unwrapped = property ? unwrapPropertyValue(value, property) : value;
  const parsed = parseJsonish(unwrapped);
  const types = schemaTypes(schema);

  if (types.includes("string") && isPlainObject(parsed)) {
    return typeof unwrapped === "string" ? unwrapped : JSON.stringify(parsed);
  }
...
function unwrapToolInput(input) {
  ...
  if (typeof next === "string") {
    next = mergePlainJsonObjects(parseJsonSequence(next)) || next;
  }
...
function normalizeToolUseInput(toolName, input, toolSchemas) {
  ...
  for (const [key, value] of Object.entries(next)) {
    ...
    if (target) {
      normalized[target] = coerceValueForSchema(value, properties[target], target);
    } else if (keepUnknown) {
